function doGet() {
  return HtmlService.createTemplateFromFile('PostTestQuestion')
    .evaluate()
    .setTitle('Add Main-Training Question')
...
function getObjectives() {
  ...
  var objectives = sheet.getRange('C5:C').getValues().filter(String);
  return objectives.map(function(row) { return row[0]; });
}

function submitPostQuestion(data) {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Post-Test Questions');
