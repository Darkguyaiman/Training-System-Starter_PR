...
var idQuestion = form.addTextItem();
  ...
  var folderId = "1oPDMqGDLf_jR5X9EMohKNRBLrWlwQB_x";
...
function getCorrectAnswers() {
  // ID of the external spreadsheet
  ...
  try {
    ...
    // Get the data from the external sheet
    ...
    // Prepare data for writing to the target sheet
    const dataToWrite = [];
    
    for (let i = 0; i < questions.length; i++) {
      if (!questions[i][0]) continue; // Skip empty rows
      
      const question = questions[i][0];
      let correctAnswer = '';
      
      // Check which option has the * symbol
